...
# convert domains csv file                                  #
#############################################################

#perl prot2genCoord_dom.pl ../data/annotations/LRR_domains_NIP.csv ../data/gff/LRR_Nipponbare.gff ./Nip_domains_genomic_pos.txt
...
while (<INFILE>) {
    ...
    if ($line=~/(.*);(.*);(.*);(.*)/) {
        ...
        while (<GFF>) {
            ...
            #positif
            if ($gffline=~/.*CDS\s+(\d+)\s+(\d+)\s+.\s+(\+).*($id).*/ && $foundPos eq "false") {
        ...
        if($foundPos eq "false"){
            ...
            foreach my $reverseline (reverse(<REVERSE> )) {
                #negatif
                if ($reverseline=~/.*CDS\s+(\d+)\s+(\d+)\s+.\s+(\-).*($id).*/ && $foundNeg eq "false") {
